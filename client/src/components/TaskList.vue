    ...
    <div class='row'>
      <div class='col-md-6 mx-auto'>
        ...
        <form v-on:submit.prevent='addNewTask'>
          ...
          <button v-if='this.isEdit==false' class='btn btn-success btn-block' >Submit</button>
        ...
        <table class='table'>
          <tr v-for='(todo) in todos' v-bind:key='todo.id' v-bind:title='todo.title'>
            ...
            <td class='text-right'>
              <button v-on:click='editTask(todo.title, todo._id)' class='btn btn-info'>Edit Task</button>

              <button v-on:click='deleteTask(todo._id)' class='btn btn-danger'>Delete Task</button>
            </td>
          </tr>
        </table>
      </div>
